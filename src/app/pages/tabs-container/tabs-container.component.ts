...
} from "src/app/store/actions/bar-chart.actions";
...
import {
  ...
  DeleteTabAction,
  UpdateTabAction,
} from "src/app/store/actions/tab.actions";
...
export class TabsContainerComponent implements OnInit {
  ...
  active: number = 0;
  data: any[] = [84, 14, 234, 37, 64, 42, 197, 11]
    .sort((a, b) => b - a)
    .map((val, index) => {
  ...
  addChartClick(tab: TabModel, posIndex: number) {
    ...
    this.store.dispatch(
      new AddBarChartAction({
        customColors: [],
        data: tab.chartData,
        tabId: tab.index,
        boudaries: [tab.slider.value, tab.slider.highValue],
      })
    );
