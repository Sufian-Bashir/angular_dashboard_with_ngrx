...
export function ChartReducer(state: ChartState = defaultChart, action: ChartAction) {
    ...
    switch (action.type) {
        ...
        case ChartActionTypes.DELETE_CHART_SUCCESS:
            return { ...state, list: state.list.filter(elem => elem.tabId !== action.payload), loading: false };
