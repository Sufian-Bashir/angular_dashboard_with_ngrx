...
export const data: any[] = [84, 14, 234, 37, 64, 42, 197, 11]
  .sort((a, b) => b - a)
  .map((val, index) => {
...
export function TabReducer(
  state: Array<TabModel> | undefined = defaulTab,
  action: TabAction
) {
  switch (action.type) {
    case TabActionTypes.ADD_TAB:
      ...
      let exists = state.some(
        (s) => s.index == (action.pyaload as TabModel).index
      );
      if (exists) {
